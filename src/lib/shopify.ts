...
const SHOPIFY_STORE_PERMANENT_DOMAIN = 'pixel-pouch-co-7ukg7.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = 'df0518cfb60cfa3eb439c7d8c1af31fd';
...
// === VVV เพิ่มใหม่: Query สำหรับดึงสินค้าจาก Collection VVV ===
...
// === Function: ดึงสินค้าทั้งหมด ===
export async function fetchProducts(first: number = 20) {
  const data = await storefrontApiRequest(STOREFRONT_QUERY, { first });
  return data?.data?.products?.edges || [];
}

// === Function: ดึงสินค้าตาม Handle ===
export async function fetchProductByHandle(handle: string) {
  const data = await storefrontApiRequest(PRODUCT_BY_HANDLE_QUERY, { handle });
  return data?.data?.productByHandle;
}

// === VVV เพิ่มใหม่: Function สำหรับดึงสินค้าจาก Collection VVV ===
export async function fetchProductsByCollection(collectionHandle: string, first: number = 50) {
  try {
    ...
    // ถ้าไม่เจอ Collection คืนค่า array ว่าง
